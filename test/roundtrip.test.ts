...
describe.each(LEGS)("$label", ({ codec }) => {
  ...
  describe("logical column types", () => {
    ...
    const id = uuid();
    const day = date();
    const clock = time({ unit: "millis", isAdjustedToUTC: false });
    const precise = time({ unit: "micros", isAdjustedToUTC: false });
    const nanos = time({ unit: "nanos", isAdjustedToUTC: false });
    const instant = timestamp({ unit: "micros", isAdjustedToUTC: true });
    const half = float16();
